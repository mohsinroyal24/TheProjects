...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (deletedProduct) {
      const updatedProducts = products.filter(product => product.slug !== deletedProduct);
    ...
    <>
      ...
      <div className="container mx-10 mt-2 p-2">
        ...
        {loading && <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="50px" height="50px">
          <circle cx="25" cy="25" r="20" fill="none" stroke="#000000" strokeWidth="4" strokeDasharray="31.42" strokeDashoffset="0">
            <animate attributeName="strokeDashoffset" dur="1.5s" repeatCount="indefinite" from="0" to="62.84" />
          </circle>
        </svg>
        }
        ...
        <div className="dropContainer mx-2 absolute w-[78.5vw] border-1 rounded-md bg-purple-100 ">
          ...
          {dropdown.map(item => {
            return <div key={item.slug}
              ...
              <div className="mx-5">
                <button onClick={() => { buttonAction("minus", item.slug, item.quantity) }} disabled={loadingaction} className="subtract cursor-pointer inline-block px-3 py-1 mr-2 bg-purple-500 text-white font-semibold rounded-lg shadow-md disabled:bg-purple-200">-</button>
                <span className="quantity">{item.quantity}</span>
                <button onClick={() => { buttonAction("plus", item.slug, item.quantity) }} disabled={loadingaction} className="add  cursor-pointer inline-block px-3 py-1 ml-2 bg-purple-500 text-white font-semibold rounded-lg shadow-md disabled:bg-purple-200">+</button>
              </div>
            </div>
          })}
        </div>
      ...
      <div className="container mx-10 mt-2 p-2">
        ...
        <div className="product-form py-3 px-2 mb-6 flex flex-wrap items-center">
          <div className="py-2 w-full md:w-8/12"> {/* 8/12 width */}
          ...
          <div className="py-2 px-2 w-full md:w-2/12"> {/* 2/12 width */}
            <button
              onClick={addProduct}
